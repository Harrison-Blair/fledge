...
func New() *cobra.Command {
	cmd := &cobra.Command{Use: "task", Short: "Create, assign, complete, verify, cancel, list, and inspect durable tasks", Args: cobra.NoArgs,
		Long: "Create, assign, complete, verify, cancel, list, and inspect durable tasks.\n\nA task moves created → assigned → completed → verified, and can be cancelled\nbefore it is verified. Records live in .fledge/state/tasks under the repository's\nprimary checkout and outlive their agents' panes. Owners and verifiers are Fledge\nagent record ids. Task status changes only through these commands; completion is\nnever derived from Herdr idle or done."}
	cmd.AddCommand(create.New(), assign.New(), complete.New(), verify.New(), cancel.New(), list.New(), get.New())
	return cmd
}
