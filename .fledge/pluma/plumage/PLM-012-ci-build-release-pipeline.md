...
4. FC-4: If the post-merge safety-net checks pass AND the `VERSION` file's contents differ from its value on the previous commit to `main`, the workflow cross-compiles the `fledge` binary for `linux/amd64`, `linux/arm64`, `darwin/amd64`, `darwin/arm64`, and `windows/amd64`, using the same ldflag version-injection `scripts/install.sh` already uses.
...
- [ ] AC-4: Merging a PR to `main` that DOES change `VERSION` (e.g. `0.4.0` → `0.4.1`) produces a GitHub Release tagged `v0.4.1` with auto-generated notes and five binary archives (`linux_amd64`, `linux_arm64`, `darwin_amd64`, `darwin_arm64`, `windows_amd64`) plus a `checksums.txt`, all attached to the release.
