...
- **Coverage (one script per command plus an integration script):**
  ...
  - `scan.txtar` — module grouping, scan-ignore filtering, JSON.
...
- **`internal/scan`** — `scan_test.go`, 3 tests: module grouping with file/byte counts, no-scan-ignore includes all, empty repo → no modules.
