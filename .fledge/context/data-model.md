...
| `ID` | string | `TASK-NNN` |
| `Title` | string | |
| `Requirement` | string | parent `REQ-NNN` |
| `Status` | string | `blocked` \| `ready` \| `in-progress` \| `done` |
| `Priority` | string | `P0`–`P3` |
| `DependsOn` | []string | blocking `TASK-NNN` IDs |
...
JSON content of a `.fledge/locks/TASK-NNN.lock` file: `{Task, Owner, PID, Created, Branch}`. Acquisition contention returns a `HeldError` wrapping the existing `Record`.
