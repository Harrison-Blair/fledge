...
## Functional Criteria
1. FC-1: `fledge report` prints task counts by status: totals for `blocked`, `ready`, `in-progress`, `done`, and overall.
...
5. FC-5: The report lists active locks: each locked task with the lock's owner, from `.fledge/locks/`.
...
- New validation rules — `fledge check` remains the validator; report only surfaces what it encounters while reading.
