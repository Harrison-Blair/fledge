...
func TestMatch(t *testing.T) {
	tests := []struct {
		...
		{"negated bang class does not cross a slash", "a[!x]b", "a/b", false, false},
		{"negated caret class does not cross a slash", "a[^x]b", "a/b", false, false},
		{"posix class matches", "[[:digit:]].go", "5.go", false, true},
		{"posix class does not match outside set", "[[:digit:]].go", "a.go", false, false},
		{"negated posix class excludes the separator", "a[^[:digit:]]b", "a/b", false, false},
		{"posix class inside a bigger class", "[[:digit:]x].go", "x.go", false, true},
		{"template pyc class", "*.py[cod]", "x.pyc", false, true},
		{"template object class", "*.[oa]", "libx.o", false, true},
		{"template case class dir", "[Bb]in/", "bin", true, true},
		{"template bak class", "*.[Bb][Aa][Kk]", "notes.BAK", false, true},
		{"template soname class", "*.so.[0-9]", "libx.so.6", false, true},
		{"escaped backslash trims the trailing space", "a\\\\ ", "a\\", false, true},
		{"negated class with bare trailing dash parses", "a[!a-]b", "a.b", false, true},
		{"negated class with bare trailing dash excludes separator", "a[!a-]b", "a/b", false, false},
		{"negated multi-member bare trailing dash parses", "a[!._-]b", "axb", false, true},
		{"bare trailing dash is a literal member not a range", "a[!.-]b", "a-b", false, false},
		{"bare trailing dash does not swallow the plus range", "a[!+-]b", "a.b", false, true},
		{"positive class does not cross a slash", "a[/x]b", "a/b", false, false},
		{"positive class still matches its members", "a[/x]b", "axb", false, true},
		{"positive class of only separators matches nothing", "a[/]b", "a/b", false, false},
