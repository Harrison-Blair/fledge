...
Foraging: run `planning.md` step 2, whose wait is defined by the **Commissioner** section of `foraging.md` — honor that contract rather than duplicating it here. Where your harness lets a worker spawn workers, spawn the forager yourself; where it does not, obtain it via `SPAWN-REQUEST`. Either way, you verify the nest output with `fledge nest status` and request the forager's shutdown by name. **In short: you `fledge await` its ledger `status` record for the done signal rather than its final message or any idle/lifecycle notification, and recover an exit-4 timeout with `fledge pulse` per that section.** On a harness where the orchestrator (not you) spawned the forager, its lifecycle notifications land on the orchestrator, which forwards them to you verbatim (`planning.md` §0) — but a forwarded idle or "worker finished" notification is never to be trusted over the ledger: it is a nudge to check, never itself proof of done or stalled. You are still the commissioner and run that wait — the forager's final coverage-summary message, when it comes, is addressed to you.
