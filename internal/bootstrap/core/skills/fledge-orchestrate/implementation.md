...
A feather is cleared for merge in one of two ways: its skua records `fledge verdict FTHR-### --result pass` (having checked every AC box in the worktree and committed that change) and sends you a stateless nudge — you determine the outcome deterministically with `fledge await FTHR-### --kind verdict --exists --timeout <duration>` (write-once record, never a change-wait; recover an exit-4 timeout with `fledge pulse <skua-name>`, per the standing pulse-recovery pattern), never by parsing the nudge itself — or, after a skua's 3rd-rejection escalation (§4), presenting the unresolved findings and cycle history — the user chooses (decision gate) to ship anyway (waiving the findings) rather than send it back for another cycle. On a user override, record the accepted (waived) findings on the feather file and use `--force` on the criteria-gated commands, so the decision is auditable. Then:
...
Workers escalate blockers and disputes to you by recording `fledge escalate <own-name> --message "<blocker>"` (per `brooder.md`/`skua.md`) and sending a stateless nudge; you detect it deterministically with `fledge await <worker-name> --kind escalation --exists --timeout <duration>` (write-once record; recover an exit-4 timeout with `fledge pulse <worker-name>`, per the standing pulse-recovery pattern) rather than relying solely on the nudge arriving, and read the blocker's substance with `fledge ledger read <worker-name> --kind escalation`. Triage by fledge's standing rule — facts belong in the repo, decisions belong to the user:
...
2. Inventory reality: `fledge broods --stale` classifies the held locks for you (it reports the per-lock `worktree` path and `worktree_exists`), plus `fledge broods` (owner, branch per held lock) and `fledge vee` for the fuller picture. Feathers with a held lock (equivalently `status: hatching`) and `worktree_exists: true` are the resume set. The locks `--stale` reports are the release candidates — release each with `fledge abandon FTHR-### --force`, then set its status explicitly (`fledge status FTHR-### pipping --force`) so it re-enters the ready set. Caveat for legacy records: a `--stale` entry whose `worktree` field is empty (a lock brooded before this repo adopted `--worktree`, i.e. predates FTHR-050) was classified stale by convention — an empty path, not a path checked and found gone — so re-check it against `git worktree list` before force-releasing; if that feather's worktree is in fact still present, treat it as a resume, not a release.
3. For each, check `fledge ledger read <old-name> --kind escalation` before respawning — a resume loses the conversation, but not the ledger: a paused brooder's blocker text, if it escalated before the resume, is still on disk and would otherwise be lost. Then respawn a fresh **pair** (a new species is fine — one species for both members) into the **existing** worktree and branch. The brooder's spawn prompt must say partial work may exist: inspect commits and the diff before continuing, re-verify the test-first evidence chain — if the captured failing-test output was lost with the old worker, re-derive it (revert/stash) or flag the gap to the skua — and pass along any recovered escalation text so a genuine blocker isn't silently dropped. The skua's spawn prompt names its paired brooder and notes an earlier skua may have already checked some AC boxes on the branch, or already recorded a `fledge verdict` for it — audit rather than assume.
