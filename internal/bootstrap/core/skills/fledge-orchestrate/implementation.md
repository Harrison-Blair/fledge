...
Workers escalate blockers and disputes to you by recording `fledge escalate <own-name> --message "<blocker>"` (per `brooder.md`/`skua.md`) and sending a stateless nudge; you detect it deterministically with `fledge await <worker-name> --kind escalation --exists --timeout <duration>` (write-once record; recover an exit-4 timeout with `fledge pulse <worker-name>` per `worker-protocols.md`) rather than relying solely on the nudge arriving, and read the blocker's substance with `fledge ledger read <worker-name> --kind escalation`. Triage by fledge's standing rule — facts belong in the repo, decisions belong to the user:
