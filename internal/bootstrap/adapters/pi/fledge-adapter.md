...
- `confirm-gate` (review): present the full material, then ask the user in chat for a structured "Accept" / "Make changes" choice (or use a `fledge_gate` extension tool if the M4 extension is installed). Loop on "Make changes" until "Accept".
