...
func init() { register("colony", runReport, "fledge colony [--json]") }

type reportCounts struct {
	Blocked    int `json:"egg"`
	Ready      int `json:"pipping"`
	InProgress int `json:"hatching"`
	Done       int `json:"fledged"`
...
type report struct {
	...
	Requirements []reqCompletion `json:"plumages"`
	Orphans      []orphanTask    `json:"orphans"`
	Blocked      []blockedTask   `json:"egg"`
	Locks        []lockEntry     `json:"broods"`
	Issues       issues          `json:"issues"`
}

func runReport(args []string) int {
	fs := flag.NewFlagSet("colony", flag.ContinueOnError)
...
// rendering. Feathers whose plumage reference does not resolve to a loaded
// plumage are surfaced as orphans and excluded from per-plumage counts.
	...
	for _, t := range set.Tasks {
		if t.Requirement != "" && set.Req(t.Requirement) == nil {
			rep.Issues.DanglingRefs = append(rep.Issues.DanglingRefs,
				fmt.Sprintf("%s plumage %s not found", t.ID, t.Requirement))
		}
		...
		for _, dep := range t.DependsOn {
			...
			} else if d.Status != spec.TaskFledged {
				unmet = append(unmet, dep)
			}
		}
		if len(unmet) > 0 {
...
func printReport(rep report) {
	c := rep.Counts
	fmt.Printf("Feathers: %d total (egg: %d, pipping: %d, hatching: %d, fledged: %d)\n",
	...
	for _, r := range rep.Requirements {
		fmt.Printf("  %s  %s  %s  %d/%d fledged\n", r.ID, r.Title, r.Status, r.Done, r.Total)
	}

	fmt.Println("\nOrphan feathers (dangling plumage reference):")
	if len(rep.Orphans) == 0 {
		fmt.Println("  (none)")
	}
	for _, o := range rep.Orphans {
		fmt.Printf("  %s  %s  (plumage: %s)\n", o.ID, o.Title, o.Requirement)
	}
