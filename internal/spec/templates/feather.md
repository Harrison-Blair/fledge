...
## Affected Modules
Modules and key files involved, citing the nest docs consulted (e.g. "see .fledge/nest/modules.md → <module>").
...
## Acceptance Criteria
Checkbox list, one `- [ ] AC-N: …` line per criterion — authored unchecked; checked only via `fledge criteria check`, with per-criterion evidence in `.fledge/molt/{{ID}}.md`. Reference the parent plumage's criteria where applicable (e.g. "satisfies {{REQ}} FC-2"). AC-1 is always:
