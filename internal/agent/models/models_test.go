...
func TestModelsDiscoversAllHarnessesSorted(t *testing.T) {
	d := libmodels.Discovery{Home: fixtureHome(t), Run: fixtureRunner().run}
	out := Run(context.Background(), d, Options{})
	if out.Status != "success" || out.Error != nil || out.Operation != "agent.models" || len(out.Effects) != 0 {
		t.Fatalf("%+v", out)
	}
	want := Result{Models: []libmodels.Row{
...
func TestModelsHarnessFilter(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.harness, func(t *testing.T) {
			...
			out := Run(context.Background(), libmodels.Discovery{Home: home, Run: runner.run}, Options{Harness: tc.harness})
			if out.Status != "success" || out.Error != nil {
				t.Fatalf("%+v", out)
			}
			got := []string{}
			for _, m := range out.Result.(Result).Models {
...
func TestModelsRejectsUnknownHarness(t *testing.T) {
	runner := fixtureRunner()
	out := Run(context.Background(), libmodels.Discovery{Home: fixtureHome(t), Run: runner.run}, Options{Harness: "nope"})
...
func TestModelsFailSoft(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			out := Run(context.Background(), libmodels.Discovery{Home: home, Run: runner.run}, Options{})
			if out.Status != "success" || out.Error != nil {
				t.Fatalf("%+v", out)
			}
			if got := out.Result.(Result).Models; len(got) != 0 {
...
func TestModelsEmptyResultIsNotNull(t *testing.T) {
	out := Run(context.Background(), libmodels.Discovery{Home: t.TempDir(), Run: (&fakeRunner{}).run}, Options{})
	var b strings.Builder
	if err := out.Write(&b, true, Render); err != nil {
		t.Fatal(err)
	}
